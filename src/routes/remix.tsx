...
function RemixPage() {
  ...
  return (
    <AppShell
      title="Remix AI"
      subtitle="Biến 1 video viral thành 3–10 phiên bản caption sẵn sàng đăng — bằng giọng văn của bạn."
    >
        ...
        <aside className="space-y-4">
          <div className="rounded-2xl border border-border/60 bg-surface-elev p-5 sticky top-24 space-y-5">
            <div>
              ...
              <div
                className={`rounded-xl aspect-[4/5] bg-gradient-to-br ${src.thumb} p-4 flex flex-col justify-between`}
              >
                <span className="text-[10px] font-bold text-white bg-black/40 backdrop-blur w-fit px-2 py-0.5 rounded-full">
                  {src.platform}
                </span>
                <div className="text-white font-display font-bold text-sm leading-snug">
                  {src.hook}
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {["TikTok", "Instagram Reels", "Threads", "Facebook"].map((p, i) => (
                  <button
                    key={p}
                    className={`text-xs px-3 py-1.5 rounded-full border ${i === 0 ? "bg-foreground text-background border-foreground" : "border-border bg-card"}`}
                  >
                    {p}
                  </button>
                ))}
        ...
        <div className="space-y-5">
          <div className="flex items-center justify-between">
            <h2 className="font-display text-xl font-bold flex items-center gap-2">
              <Sparkles className="h-5 w-5 text-brand" /> 3 phiên bản remix
            </h2>
