...
function CalendarPage() {
  return (
    <AppShell
      ...
      subtitle="AI cân đối hook / story / educate / sell để bạn không lặp ý."
    >
      <div className="flex flex-wrap items-center gap-3 mb-6">
        ...
        <div className="flex flex-wrap gap-2 ml-auto">
          ...
          <Button size="sm" className="bg-gradient-brand text-white shadow-glow gap-2">
            <Sparkles className="h-4 w-4" /> Tạo lịch mới bằng AI
          </Button>
        </div>
      </div>
        ...
        <div className="grid grid-cols-7 gap-1">
          {Array.from({ length: 30 }).map((_, i) => {
            ...
            return (
              <div
                key={i}
                className="aspect-[3/4] rounded-xl border border-border/60 p-2.5 bg-card hover:shadow-card transition flex flex-col gap-1.5"
              >
                ...
                <span
                  className={`text-[10px] font-semibold uppercase tracking-wider w-fit px-1.5 py-0.5 rounded border ${typeColors[item.type]}`}
                >
                  {item.type}
                </span>
                <div className="text-[11px] leading-tight line-clamp-3 text-foreground/80">
                  {item.title}
                </div>
              </div>
            );
