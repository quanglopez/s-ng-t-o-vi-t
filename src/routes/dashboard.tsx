          ...
          <section>
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              {posts.slice(0, 4).map((p) => (
                ...
                >
                  <div
                    className={`aspect-[16/10] bg-gradient-to-br ${p.thumb} p-4 flex flex-col justify-between`}
                  >
                    <div className="flex items-center justify-between">
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-muted-foreground mt-1">
                  Niche Beauty đang nổi hook dạng "Tôi đã bỏ X triệu để…". Bạn có 3 video lưu dùng
                  được công thức này.
                </p>
          ...
          <section className="rounded-2xl border border-border/60 bg-surface-elev p-5">
            ...
            <div className="space-y-3">
              {boards.slice(0, 4).map((b) => (
                ...
                >
                  <div
                    className={`h-10 w-10 rounded-lg bg-gradient-to-br ${b.color} grid place-items-center text-lg`}
                  >
                    {b.emoji}
                  </div>
          ...
          <section className="rounded-2xl border border-border/60 bg-surface-elev p-5">
            ...
            <div className="mt-4 flex gap-1">
              {Array.from({ length: 14 }).map((_, i) => (
                <div
                  key={i}
                  className={`h-7 flex-1 rounded ${i < 12 ? "bg-gradient-brand" : "bg-muted"}`}
                />
              ))}
            </div>
          </section>
